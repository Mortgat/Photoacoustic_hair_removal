...
class PUStrictDataset(Dataset):
    def __init__(self, vol, dist, frangi, pos, hn, easy_neg, surf, p_xy=PATCH_XY, p_z=PATCH_Z, iters=400):
        self.vol, self.dist, self.frangi = vol, dist, frangi
        self.pos, self.hn, self.easy_neg, self.surf = pos, hn, easy_neg, surf
    ...
    def __getitem__(self, idx):
        ...
        def get_patch(x, y, z):
            ...
            x_p = np.stack([self.vol[xs, ys, zs], self.dist[xs, ys, zs], self.frangi[xs, ys, zs]], axis=0)
...
def main():
    ...
    del surf_propre, dist_mm
    gc.collect()

    dataset = PUStrictDataset(vol, dist_norm, frangi, pos, hn, easy_neg, surf)
    loader = DataLoader(dataset, batch_size=1, shuffle=True)

    model = StandardUNet3DAniso(in_channels=3, base=32).to(device)
    ...
    del dataset, loader, optimizer, loss, logits
    gc.collect()
