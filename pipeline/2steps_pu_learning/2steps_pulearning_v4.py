...
# ==========================================
# CONFIGURATION V5.4 (BINAIRE STRICT + SPATIAL DROPOUT)
# ==========================================
...
# ==========================================
# ARCHITECTURE U-NET 3D AVEC SPATIAL DROPOUT
# ==========================================
...
class StandardUNet3DAniso(nn.Module):
    def __init__(self, in_channels=1, base=32, dropout_p=0.2): 
        super().__init__()
        ...
        # Piste 4 : Spatial Dropout au Bottleneck
    ...
    def forward(self, x):
        ...
        e4 = self.dropout(e4) # Désactivation spatiale aléatoire
...
class PUStrictDataset(Dataset):
    def __init__(self, vol, pos, hn, vs, p_xy=PATCH_XY, p_z=PATCH_Z, iters=400):
        self.vol = vol
        self.pos, self.hn, self.vs = pos, hn, vs
        self.p_xy, self.p_z, self.iters = p_xy, p_z, iters
        self.nx, self.ny, self.nz = vol.shape
        
        # Retour à la logique booléenne stricte
    ...
    def __getitem__(self, idx):
        ...
        def get_patch(x, y, z):
            ...
            x_p = np.expand_dims(self.vol[xs, ys, zs], axis=0)
            ...
            # Application binaire pure
            target[0][self.pos[xs, ys, zs]] = 1.0
            valid[0][self.pos[xs, ys, zs]] = 1.0 
            
            target[0][self.hn[xs, ys, zs]] = 0.0
            valid[0][self.hn[xs, ys, zs]] = 1.0
            
            target[0][self.vs[xs, ys, zs]] = 0.0
            valid[0][self.vs[xs, ys, zs]] = 1.0
        ...
        if len(self.vs_coords) > 0:
            ...
            hx2, hy2, hz2 = extract(self.hn_coords)
            dx, dy, dv = get_patch(hx2, hy2, hz2)
...
def sliding_window_inference_ram_safe(model, vol, device, p_xy=PATCH_XY, p_z=PATCH_Z):
    model.eval()
    nx, ny, nz = vol.shape
        ...
        for x in tqdm(range(0, max(1, nx - p_xy + 1), step_xy), desc="Inférence X"):
            for y in range(0, max(1, ny - p_xy + 1), step_xy):
                for z in range(0, max(1, nz - p_z + 1), step_z):
                    ...
                    patch_in = np.expand_dims(vol[xs, ys, zs], axis=0)
...
def main():
    ...
    # Chargement en booléen strict
    pos = np.load(os.path.join(DOSSIER_MASQUES, "masque_poils_surs.npy")).astype(bool)
    hn = np.load(os.path.join(DOSSIER_MASQUES, "masque_hard_negatives.npy")).astype(bool)
    
    f_vs = os.path.join(DOSSIER_MASQUES, "masque_vaisseaux_surs.npy")
    vs = np.load(f_vs).astype(bool) if os.path.exists(f_vs) else np.zeros_like(hn)

    dataset = PUStrictDataset(vol, pos, hn, vs)
    loader = DataLoader(dataset, batch_size=1, shuffle=True)

    model = StandardUNet3DAniso(in_channels=1, base=32, dropout_p=0.2).to(device)
    ...
    masque_predit = sliding_window_inference_ram_safe(model, vol, device)
