...
def rgba_overlay_prob(pred_2d, color_rgb, prob_mode):
    ...
    if prob_mode:
        alpha = (pred_2d * 200).clip(0, 255).astype(np.uint8)
...
class SniperDiagnosticTool(QtWidgets.QWidget):
    def __init__(self, vol, surf, pos, pred_map, mip_norm, vol_filtre, vaisseaux_mask):
        ...
        # Gestion des masques float (Lissage Gaussien)
        self.pos_mask = pos.astype(np.float32)
        ...
        # Rendu 2D du maximum pour respecter le flou
        self.pos_xy = np.max(self.pos_mask, axis=2).T
    ...
    def init_ui(self):
        ...
        self.label_3d = QtWidgets.QLabel("Volume 3D Local (Synchronisé)")
    ...
    def update_mip_display(self):
        ...
        # Affiche le flou des poils via opacité
        self.xy_pos.setImage(rgba_overlay_prob(self.pos_xy, (255, 255, 0), prob_mode) if self.cb_pos.isChecked() else empty_xy_rgba, autoLevels=False)
        
        # Vaisseaux binaires
        vaisseaux_prob = self.vs_xy.astype(np.float32)
        self.xy_vs.setImage(rgba_overlay_prob(vaisseaux_prob, (255, 0, 255), False) if self.cb_vs.isChecked() else empty_xy_rgba, autoLevels=False)
    ...
    def refresh_3d(self):
        if self.last_cx is None or self.last_cy is None: return
    ...
    def extract_and_render_3d_patch(self, xmin, xmax, ymin, ymax, cx, cy):
        ...
        # Extraction binaire pour la 3D (seuil à 0.1 pour gommer la queue du gaussien)
        surs_patch = self.pos_mask[xmin:xmax, ymin:ymax, :] > 0.1
        ...
        show_pos = self.cb_pos.isChecked()
        show_vs = self.cb_vs.isChecked()
        show_pred = not self.radio_off.isChecked()
        
        if show_pos: rgba[flat_surs] = [255, 255, 0, 0]
        if show_pred: rgba[flat_pu] = [255, 0, 0, 0]
        if show_vs: rgba[flat_vs] = [255, 0, 255, 0]
        rgba[flat_skin] = [0, 255, 0, 0]

        alpha = (norm_vol**2 * 150).astype(np.ubyte) 
        mask_any = flat_skin
        if show_pos: mask_any = mask_any | flat_surs
        if show_pred: mask_any = mask_any | flat_pu
        if show_vs: mask_any = mask_any | flat_vs
