...
# Paramètres Stricts - Hard Negatives Vasculaires
PERCENTILE_FRANGI_VAISSEAUX = 95.0  # Top 5% uniquement
...
def main():
    ...
    print("🟢 [3/5] Extraction des Poils Sûrs...")
    ...
    print("🔴 [4/5] Extraction des Hard Negatives...")
    
    # 1. Bruit de Fond
    seuil_hn_min = np.percentile(mip_norm, 50)
    seuil_hn_max = np.percentile(mip_norm, 80)
    ...
    hn_2d_mask = (mip_norm > seuil_hn_min) & (mip_norm < seuil_hn_max) & (f2d < 0.05) & (~poils_surs_2d)
    ...
    # 2. Vaisseaux Sûrs (Géométrie Horizontale + Profondeur Stricte)
    seuil_f_vs = np.percentile(f2d, PERCENTILE_FRANGI_VAISSEAUX)
    cand_vs = (f2d > seuil_f_vs) & (~poils_surs_2d)
    labels_vs = label(cand_vs)
    masque_vaisseaux_3d = np.zeros_like(vol_brut, dtype=bool)
    ...
    for reg in regionprops(labels_vs):
        if reg.major_axis_length >= (LONGUEUR_MIN_VAISSEAU_MM / DX_MM):
            ...
            # Validation stricte par rapport à la peau
            z_skin_valid = surface_peau[xs, ys]
            ...
            if np.sum(mask_nan) > 0:
                ...
                if z_median >= (skin_median + profondeur_min_voxels):
                    ...
                    longueur_xy = max(dx, dy)
                    
                    if longueur_xy > 0 and (dz / longueur_xy) <= RATIO_HORIZONTALITE_MAX:
                        masque_vaisseaux_3d[xs, ys, z_vals] = True
